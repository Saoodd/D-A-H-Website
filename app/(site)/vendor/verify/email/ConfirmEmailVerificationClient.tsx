      ...
      <div className="w-full max-w-sm text-center flex flex-col gap-8">
        ...
        {(state === "verified" || state === "already") && (
          <div>
            ...
            <div className="mt-6">
              <LinkButton href="/vendor/verify" size="md">
                {isAr ? "متابعة التحقق" : "Continue Verification"}
              </LinkButton>
            </div>
          </div>
        )}
        ...
        <Link href="/vendor/verify" className="text-sm text-brown-dark underline underline-offset-2 hover:text-brown">
          {isAr ? "الذهاب لصفحة التحقق" : "Go to Verify Your Account"}
        </Link>
      </div>
    </div>
  );
