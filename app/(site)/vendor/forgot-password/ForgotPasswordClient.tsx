...
export function ForgotPasswordClient() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // request succeeded on the wire — the account's existence is never
      // revealed, and a network hiccup shouldn't leak that either.
      setSubmitting(false);
      setSent(true);
    }
  }
        ...
        {sent ? (
          <div className="flex flex-col gap-8 text-center">
            <div>
              ...
              <p className="text-sm text-brown-light leading-relaxed">
                {locale === "ar"
                  ? "إذا كان هناك حساب مرتبط بهذا البريد، فقد أرسلنا إليك تعليمات إعادة تعيين كلمة المرور."
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-8">
            <div className="text-center">
              <h1 className="font-heading text-3xl text-brown-dark mb-3">{locale === "ar" ? "نسيت كلمة المرور؟" : "Forgot password?"}</h1>
